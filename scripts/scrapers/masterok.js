...
/**
 * Основной путь: JSON-LD Product (надёжно на company-site Prom.ua)
 */
...
/**
 * Запасной путь: карточки data-qaid="product-block"
 */
function extractFromBlocks($, categoryPath, existingMap) {
  ...
  $('[data-qaid="product-block"]').each((_, el) => {
    ...
    const $link = $card.find('a.cs-goods-title, a.cs-image-holder__image-link').first();
    ...
    const priceText =
      $card.find('.cs-goods-price__value_type_current, .cs-goods-price__value').first().text() ||
    ...
    const skuText = $card.find('.cs-goods-sku').first().text().replace(/код:?/i, '').trim();
...
export async function scrapeMasterokCategory(categoryPath) {
  // поддержка ?page=2 или /page_2 если появится
  ...
  for (const path of pagesToTry) {
    ...
    try {
      ...
      if (merged.size === 0 && path !== categoryPath) {
        // вторая страница пустая — стоп
        break;
      }
      ...
      // если первая страница уже мало товаров — page_2 скорее всего нет
      if (path === categoryPath && merged.size < 10) break;

      await new Promise((r) => setTimeout(r, 800));
    } catch (err) {
      if (path !== categoryPath) break; // page_2 404 — ок
  ...
  for (const product of products) {
    try {
      ...
      console.log(
        `✓ ${product.name.slice(0, 55)} — \( {product.price ? product.price + ' грн' : 'нет цены'} [id= \){product.sourceId}]`
      );
    } catch (err) {
