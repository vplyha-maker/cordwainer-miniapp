  ...
  const triggerHaptic = (style: 'light' | 'medium' | 'rigid' = 'light') => {
    try {
      ...
      if (tg?.HapticFeedback) {
        if (style === 'rigid') tg.HapticFeedback.impactOccurred('rigid')
  ...
  return (
    ...
    >
      ...
      <div className="relative z-20 flex items-center justify-between px-4 pt-3 pb-1">
        ...
        <button className="w-10 h-10 flex items-center justify-center rounded-full bg-white/5 text-[#C6A47A] active:scale-90 transition-transform">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-36 scrollbar-hide">
          ...
          {(['men', 'women', 'kids'] as Gender[]).map((g) => (
            <button
              ...
              className={`flex-1 py-2.5 rounded-[14px] text-[13px] font-medium transition-all duration-200 ${
                gender === g
                  ? 'bg-[#C6A47A]/18 text-[#E8C9A0] shadow-sm'
                  : 'text-[#6B635C]'
              }`}
            >
              ...
              {(['cm', 'mm'] as const).map((u) => (
                <button
                  ...
                  className={`px-3 py-1 rounded-full text-[11px] font-semibold transition-all ${
                    unit === u ? 'bg-[#C6A47A] text-[#0F0D0B]' : 'text-[#6B635C]'
                  }`}
                >
                  {u === 'cm' ? t.cm : t.mm}
                </button>
          ...
          <div className="text-center mb-7" onClick={startEdit}>
            {isEditing ? (
              <input
                ...
                className="w-40 text-center text-[52px] font-light text-[#E8C9A0] bg-transparent outline-none border-b border-[#C6A47A]/40"
                inputMode="decimal"
              />
            ) : (
              <div className="cursor-pointer active:opacity-70 transition-opacity">
                <span className="text-[52px] font-light tracking-tight text-[#E8C9A0] leading-none">
                  {displayValue}
                </span>
          ...
          <div className="px-1">
            <input
              ...
              className="w-full h-2 appearance-none bg-transparent cursor-pointer
                ...
                [&::-webkit-slider-thumb]:shadow-[0_0_16px_rgba(198,164,122,0.45)]
        ...
        {/* Result card — hero */}
        <div className="rounded-3xl bg-[#161311] border border-[#C6A47A]/15 p-6 mb-4 text-center">
          ...
          <div className="text-[64px] font-light text-[#E8C9A0] leading-none tracking-tight mb-1">
            {formatSize(result.eu)}
          </div>
          <div className="text-[12px] text-[#6B635C] mb-5">{t.recommended}</div>

          {/* Secondary sizes in one row */}
        ...
        <button
          ...
          className="flex items-center gap-1.5 text-[12px] text-[#C6A47A] font-medium mb-4 px-1"
        >
          ...
          {showStandards && (
            ...
            >
              <div className="rounded-2xl bg-[#161311] border border-white/[0.04] p-4">
                <div className="text-[11px] font-medium text-[#C6A47A] tracking-wide uppercase mb-3">
                  {t.standardsTitle}
                </div>
                <div className="space-y-2 text-[12px] text-[#8F867E]">
                  <div className="flex gap-3">
                    <span className="text-[#C6A47A] w-[72px] shrink-0">EU / UKR</span>
                    <span>ISO 19407 · Paris Point</span>
                  </div>
                  <div className="flex gap-3">
                    <span className="text-[#C6A47A] w-[72px] shrink-0">UK</span>
                    <span>ISO 19407 · Barleycorn</span>
                  </div>
                  <div className="flex gap-3">
                    <span className="text-[#C6A47A] w-[72px] shrink-0">US</span>
                    <span>ISO 19407 · UK + offset</span>
                  </div>
                  <div className="flex gap-3">
                    <span className="text-[#C6A47A] w-[72px] shrink-0">Mondopoint</span>
      ...
      <div className="fixed bottom-[72px] left-0 right-0 px-4 z-40">
        <button
          ...
          className="w-full py-3.5 rounded-2xl bg-[#C6A47A] text-[#0F0D0B] text-[14px] font-semibold flex items-center justify-center gap-2 active:scale-[0.98] transition-transform shadow-[0_8px_24px_rgba(198,164,122,0.25)]"
        >
