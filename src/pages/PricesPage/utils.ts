...
export function computeGroupSignals(
  ...
  if (macroIndicators.length > 0) {
    // Выравниваем кодировку (NFC) для правильного чтения 'й', 'і', 'ї' и переводим в нижний регистр
    ...
    // ДОБАВЛЕНО 'наїр' и 'гумов' для полного охвата украинского языка
    const isRubber = searchString.includes('наїр') || searchString.includes('найр') || searchString.includes('наир') || searchString.includes('nairit') || searchString.includes('гумов') || searchString.includes('резинов') || searchString.includes('каучук') || searchString.includes('sar 20') || searchString.includes('sar20') || searchString.includes('sar-20')
