...
// Добавлено четвертое состояние 'collaboration' для экрана визитки
...
export function BlogPage({ onBack, lang, isFavorite = false, onToggleFavorite }: BlogPageProps) {
  ...
  // Состояние для кнопки копирования email
  const [copied, setCopied] = useState(false)

  const hasNew = BLOG_ARTICLES.some((a) => a.isNew)
  const count = BLOG_ARTICLES.length

  // Функция тактильного отклика для кнопок
  const tactileFeedback = () => {
    if (typeof window !== 'undefined' && window.navigator && window.navigator.vibrate) {
      window.navigator.vibrate(40) // Короткая, приятная вибрация
    }
  }
  ...
  const handleCopyEmail = async () => {
    tactileFeedback()
    try {
      await navigator.clipboard.writeText('cordwain@tuta.io')
      setCopied(true)
  ...
  const t = {
    ru: {
      ...
      // Обновленные тексты для сотрудничества
      collab: 'Сотрудничество',
      collabSub: 'Предложить идею',
      collabTitle: 'Открыты к партнерству',
      collabText: 'Мы всегда рады новым идеям, нестандартным проектам и профессиональному сотрудничеству в сфере дизайна и производства.',
      emailLabel: 'ДЛЯ СВЯЗИ',
      copyBtn: 'Скопировать',
    ...
    uk: {
      ...
      // Обновленные тексты для сотрудничества (укр)
      collab: 'Співпраця',
      collabSub: 'Запропонувати ідею',
      collabTitle: 'Відкриті до партнерства',
      collabText: 'Ми завжди раді новим ідеям, нестандартним проєктам та професійній співпраці у сфері дизайну та виробництва.',
      emailLabel: 'ДЛЯ ЗВʼЯЗКУ',
      copyBtn: 'Скопіювати',
      ...
      <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="relative z-10 flex-1 flex flex-col justify-end px-4 pb-6 pt-16">
              ...
              <div className="grid grid-cols-3 gap-2.5 mb-6 items-stretch">
                {/* Читать статьи */}
                <motion.button 
                  onClick={() => { tactileFeedback(); setView('journal') }} 
                  whileTap={{ scale: 0.94 }} 
                ...
                {/* Сотрудничество (ОБНОВЛЕНО) */}
                <motion.button 
                  onClick={() => { tactileFeedback(); setView('collaboration') }} 
                  whileTap={{ scale: 0.94 }} 
                  className="relative rounded-xl p-3 text-left flex flex-col justify-between overflow-hidden cursor-pointer w-full min-h-[115px]" 
                  style={cardStyle}
                >
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center mb-2 shrink-0 relative z-10" style={{ background: 'rgba(96,165,250,0.15)', color: '#60A5FA', boxShadow: '0 0 12px rgba(96,165,250,0.2)' }}>
                    {/* Иконка лампочки - Идея/Сотрудничество */}
                  ...
                  <div className="relative z-10 flex-1 flex flex-col justify-end">
                    <div className="text-[11px] font-semibold leading-tight text-[#F5F1EB]">{t.collab}</div>
                    <div className="text-[9px] text-[#B9ACA0] mt-1 leading-snug">{t.collabSub}</div>
                  </div>
                </motion.button>

                {/* Избранное */}
                <motion.button 
                  onClick={() => { tactileFeedback(); onToggleFavorite?.() }} 
                  whileTap={{ scale: 0.94 }} 
                  className="relative rounded-xl p-3 text-left flex flex-col justify-between overflow-hidden cursor-pointer w-full min-h-[115px]" 
                  style={cardStyle}
                >
              ...
              <button 
                onClick={() => { tactileFeedback(); onBack?.() }} 
        ...
        {view === 'collaboration' && (
          <motion.div
            ...
            transition={{ duration: 0.3 }}
            className="absolute inset-0 z-40 flex flex-col items-center justify-center bg-[#151210] px-6"
          >
            {/* Легкое свечение на фоне */}
            <div className="absolute inset-0 z-0 pointer-events-none opacity-20" style={{ background: 'radial-gradient(circle at 50% 40%, #60A5FA 0%, transparent 60%)' }} />

            <div className="relative z-10 w-full max-w-[320px] flex flex-col items-center text-center">
              
              <div className="w-14 h-14 rounded-2xl mb-6 flex items-center justify-center" style={{ background: 'rgba(96,165,250,0.1)', boxShadow: '0 0 20px rgba(96,165,250,0.15)', border: '1px solid rgba(96,165,250,0.2)' }}>
              ...
              <h2 className="font-display text-[1.6rem] text-[#F5F1EB] mb-3 leading-tight">{t.collabTitle}</h2>
              <p className="text-[12px] text-[#B9ACA0] leading-relaxed mb-8">
                {t.collabText}
              </p>

              {/* Карточка с почтой */}
              <div className="w-full rounded-2xl p-5 mb-5 flex flex-col items-center justify-center relative overflow-hidden" style={cardStyle}>
                <div className="text-[9px] tracking-[0.15em] uppercase text-[#60A5FA] mb-2 font-medium">{t.emailLabel}</div>
                <div className="text-[1.2rem] tracking-wider text-[#F5F1EB] font-light font-display">
                  cordwain@tuta.io
                </div>
              </div>
              ...
              <div className="flex gap-3 w-full">
                <motion.button
                  ...
                  className="flex-1 py-3.5 rounded-xl text-[12px] font-medium transition-colors border flex items-center justify-center"
                  style={{
                    background: copied ? 'rgba(74,222,128,0.1)' : 'rgba(255,255,255,0.03)',
                    borderColor: copied ? 'rgba(74,222,128,0.3)' : 'rgba(185,172,160,0.15)',
                    color: copied ? '#4ADE80' : '#B9ACA0'
                  }}
                >
                ...
                <motion.a
                  ...
                  whileTap={{ scale: 0.94 }}
                  onClick={tactileFeedback}
                  className="flex-1 py-3.5 rounded-xl text-[12px] font-medium transition-colors flex items-center justify-center"
                  style={{
                    background: '#F5F1EB',
                    color: '#151210',
                    boxShadow: '0 4px 14px rgba(245,241,235,0.15)'
                  }}
                >
                  {t.mailBtn}
                </motion.a>
            ...
            <div className="px-4 shrink-0">
              ...
              <div className="flex overflow-x-auto gap-2 pb-2 -mx-4 px-4 scrollbar-hide snap-x">
                {t.filters.map((filter) => (
                  <button
                    key={filter.id}
                    onClick={() => { tactileFeedback(); setActiveFilter(filter.id) }}
        ...
        {view === 'article' && activeArticle && content && (
          ...
          >
            <div className="relative w-full h-[40vh] shrink-0">
              ...
              />
              <div className="absolute inset-0" style={{ background: 'linear-gradient(to bottom, rgba(21,18,16,0.1) 0%, #151210 100%)' }} />Отличный выбор. Нейтрально-профессиональный тон идеально подходит для серьезных предложений, а формат «электронной визитки» с плавной анимацией покажет ваш уровень внимания к деталям. 

Поскольку ранее вы упоминали, что предпочитаете **тактильный отклик (вибрацию)**, я добавил в код вызов легкой вибрации при нажатии на кнопки (копирование почты, открытие экрана), чтобы взаимодействие с «визиткой» ощущалось физически приятно в Telegram.

Логика работы экрана «Сотрудничество»:
1. **Плавное появление:** Экран открывается с элегантным масштабированием (scale) и размытием, фокусируя внимание на предложении.
2. **Профессиональный текст:** Коротко, емко и по делу — о том, к чему вы открыты.
3. **Удобство связи:** Почта `cordwain@tuta.io` выделена в отдельный блок. Есть две кнопки: скопировать адрес в буфер обмена (с изменением текста на «Скопировано» и тактильным откликом) и сразу открыть почтовый клиент (`mailto:`).

Вот полностью готовые файлы.

### 1. Главный файл интерфейса: `BlogPage.tsx`
...
// Добавлено состояние 'collaboration' для экрана визитки
type ViewState = 'cover' | 'journal' | 'article' | 'collaboration'
...
export function BlogPage({ onBack, lang, isFavorite = false, onToggleFavorite }: BlogPageProps) {
  ...
  // Функция для тактильного отклика (Telegram WebApp API или стандартное API браузера)
  const triggerHaptic = () => {
  ...
  const handleCopyEmail = () => {
    triggerHaptic()
    navigator.clipboard.writeText('cordwain@tuta.io')
    setEmailCopied(true)
  ...
  const t = {
    ru: {
      ...
      readBtn: 'Читать',
      // Тексты для экрана сотрудничества
